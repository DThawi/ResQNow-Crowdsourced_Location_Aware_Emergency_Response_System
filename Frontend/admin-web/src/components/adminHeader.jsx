...
const AdminHeader = ({ title }) => {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'CRITICAL', text: 'Critical Fire reported in Sector 4', time: '2m ago', read: false, targetPath: '/verification', icon: <AlertTriangle color="#D62828" size={18} /> },
    { id: 2, type: 'INFO', text: 'Unit 04 has arrived on scene', time: '5m ago', read: false, targetPath: '/incident', icon: <AlertCircle color="#3B82F6" size={18} /> },
    { id: 3, type: 'SUCCESS', text: 'Chemical spill at Hwy 95 cleared', time: '12m ago', read: false, targetPath: '/dashboard', icon: <Info color="#10B981" size={18} /> }
  ]);
    ...
    <header className="h-[85px] bg-white flex items-center justify-between px-10 shadow-sm sticky top-0 z-[100]">
      ...
      <div className="flex items-center gap-6">
          ...
          {showNotif && (
            <div className="absolute top-[60px] right-0 w-[340px] bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in duration-200">
              <div className="p-4 border-b border-slate-100 flex justify-between items-center font-extrabold text-sm">
                ...
                <span onClick={markAllAsRead} className="text-[11px] text-[#D62828] cursor-pointer font-extrabold hover:underline">
                  Mark all as read
                </span>
              </div>
                  ...
                  notifications.map(n => (
                    <div 
                      ...
                      className={`p-4 flex gap-3 border-b border-slate-50 cursor-pointer transition-colors items-center border-l-4 ${
                        n.read ? 'bg-white border-l-transparent' : 'bg-slate-50 border-l-[#D62828] hover:bg-slate-100'
                      }`}
                    >
                      <div className="mt-0.5">{n.icon}</div>
                      <div className="flex-1">
                        <div className={`text-[13px] ${n.read ? 'font-medium text-slate-500' : 'font-bold text-slate-800'}`}>
                          {n.text}
                        </div>
                        <div className="text-[11px] text-slate-400 mt-0.5">{n.time}</div>
                      </div>
                      {n.read ? <CheckCircle size={14} className="text-emerald-500" /> : <div className="w-2.5 h-2.5 rounded-full bg-[#D62828] shadow-sm"></div>}
                    </div>
                  ))
                ) : (
                  <div className="p-8 text-center text-slate-400 text-[13px]">No notifications available</div>
                )}
              </div>
              ...
              <div 
                ...
                onClick={() => { setShowNotif(false); setIsModalOpen(true); }}
              >
                View All Alerts
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 border-l border-gray-200 pl-6 cursor-pointer">
          <div className="w-10 h-10 rounded-full bg-[#2B2D42] text-white flex items-center justify-center font-bold">JA</div>
          <div className="flex flex-col text-left">
            <span className="text-sm font-bold text-[#2B2D42]">John Anderson</span>
            <span className="text-[11px] text-[#8D99AE] uppercase font-bold tracking-wide">Head Dispatcher</span>
          </div>
        </div>
      </div>
